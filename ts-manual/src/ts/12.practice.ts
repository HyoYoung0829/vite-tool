...
function rem(pxValue:string | number ,base:number = 0){
...
const movePage = (url:string,success:(url:string) => void,fail: (error: { message: string }) => void) =>{
...
type Date<T> = {success:boolean; data:T} 

const userData:Date<{name:string; age:number}> = {
  success:true,
...
type ElementType<T> = T extends (infer U)[] ? [] : never;

function logoFirst<T>(arr:T[]):void{
    const first = arr[0];
    console.log(first);

}
logoFirst(['apple', 'banana']);
logoFirst([1,2,3])
